...
contract TomiQuery {
    ...
    function queryLiquidityList() public view returns (Liquidity[] memory liquidity_list) {
        uint count = ITomiFactory(factory).getPlayerPairCount(msg.sender);
        if(count > 0) {
            liquidity_list = new Liquidity[](count);
            for(uint i = 0;i < count;i++) {
                Liquidity memory l;
                l.pair = ITomiFactory(factory).playerPairs(msg.sender, i);
    ...
    function generateProposal(address ballot_address) public view returns (Proposal memory proposal){
        ...
        proposal.totalReward = ITomiGovernance(governance).rewardOf(ballot_address);
